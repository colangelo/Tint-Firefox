// background.js - Enhanced version with color picker
...
class ThemeManager {
    ...
    async initializeWasm() {
        try {
            // Optional: Load WASM module for advanced color processing
            const response = await fetch(browser.runtime.getURL('color_processor.wasm'));
    ...
    async applyTheme(windowId, customColor = null) {
        ...
        if (customColor) {
            color = customColor;
            // Use WASM for optimal contrast calculation if available
            if (this.wasmModule) {
                textColor = this.wasmModule.get_contrast_color(color);
            } else {
    ...
    // Fallback JS implementation for contrast calculation
...
// Context menu for quick color access
browser.contextMenus.create({
    id: "pick-window-color",
    title: "Change window color",
    contexts: ["browser_action"]
});

browser.contextMenus.onClicked.addListener((info, tab) => {
    if (info.menuItemId === "pick-window-color") {
        browser.runtime.sendMessage({
            action: 'openColorPicker',
